    ...
    for i in range(starting_step, starting_step+nb_steps):
        ...
        for j in range(k_g):
            ...
            if with_class:
                ...
                loss = combined_model([z, labels], training=True) 

            else:
                labels = np.zeros((z.shape[0], 1))
                loss = combined_model(z, training=True)  
...
def adversarial_training_WGAN(arguments, train, data_cols, label_cols=[], seed=0, starting_step=0):
    ...
    _z = tf.Variable(tf.random.normal(shape=(batch_size, rand_dim)), dtype=tf.float32)

    
    _labels = None
    if with_class:  
        _x = tf.Variable(tf.random.normal(shape=(batch_size, data_dim + label_dim)), dtype=tf.float32)
        _labels = tf.Variable(tf.random.normal(shape=(batch_size, label_dim)), dtype=tf.float32)
        _g_z = generator_model(inputs=[_z, _labels]) # updated for class    
    else:      
        _x = tf.Variable(tf.random.normal(shape=(batch_size, data_dim + label_dim)), dtype=tf.float32)
        _g_z = generator_model(_z)
    
    epsilon = tf.Variable(tf.random.uniform(shape=(batch_size, 1)), dtype=tf.float32)
    ...
    adam = optimizers.Adam(learning_rate=learning_rate, beta_1=0.5, beta_2=0.9)
...
def training_steps_GAN(model_components):
    ...
    for i in range(starting_step, starting_step + nb_steps):
        ...
        for j in range(k_d):
            np.random.seed(i + j)
            z = np.random.normal(size=(batch_size, rand_dim))
            real_data = get_data_batch(train, batch_size, seed=i + j)
            
            if with_class:
                real_labels = real_data[:, -label_dim:]
                generated_data = generator_model.predict([z, real_labels])
                # Concatenate real_data and real_labels for discriminator input
                real_input = np.concatenate([real_data, real_labels], axis=1)
                generated_input = np.concatenate([generated_data, real_labels], axis=1)
            else:
                real_labels = np.zeros((batch_size, 1))  # Dummy labels for non-conditional GAN
                generated_data = generator_model.predict([z, real_labels])
                real_input = real_data  # No labels to concatenate
                generated_input = generated_data

            # Train discriminator on real data
            d_l_r = discriminator_model.train_on_batch(real_input, np.ones((batch_size, 1)))
            # Train discriminator on generated data
            d_l_g = discriminator_model.train_on_batch(generated_input, np.zeros((batch_size, 1)))
        ...
        for j in range(k_g):
            ...
            if with_class:
                labels = get_data_batch(train, batch_size, seed=i + j)[:, -label_dim:]
                loss = combined_model.train_on_batch([z, labels], np.ones((batch_size, 1)))
            else:
                labels = np.zeros((z.shape[0], 1))  # Dummy labels for non-conditional GAN
                loss = combined_model.train_on_batch([z, labels], np.ones((batch_size, 1)))
            
            combined_loss.append(loss)

        # Determine xgb loss each step, after training generator and discriminator
        if not i % 10:  # Test every 10 steps
            training = False  # 0 = test
            test_size = 492  # Test using all of the actual fraud data
            x = get_data_batch(train, test_size, seed=i)
            z = np.random.normal(size=(test_size, rand_dim))
        ...
        if not i % log_interval:
            print(f'Step: {i} of {starting_step + nb_steps}.')
            training = False  # 0 = test

            # Loss summaries
            print(f'Losses: G, D Gen, D Real, Xgb: {combined_loss[-1]:.4f}, {disc_loss_generated[-1]:.4f}, {disc_loss_real[-1]:.4f}, {xgb_losses[-1]:.4f}')
            print(f'D Real - D Gen: {disc_loss_real[-1] - disc_loss_generated[-1]:.4f}')

            if show:
                PlotData(x, g_z, data_cols, label_cols, seed=0, with_class=with_class, data_dim=data_dim, 
                        save=False, prefix=f'{data_dir}{cache_prefix}_{i}')

            # Save model checkpoints periodically (overwrite)
            model_checkpoint_base_name = f'{data_dir}{cache_prefix}_latest_model_weights.h5'
            generator_model.save_weights(model_checkpoint_base_name.format('generator'))
            discriminator_model.save_weights(model_checkpoint_base_name.format('discriminator'))

            # Optionally save losses
            losses_file = f'{data_dir}{cache_prefix}_latest_losses.pkl'
            pickle.dump([combined_loss, disc_loss_generated, disc_loss_real, xgb_losses], open(losses_file, 'wb'))

        # Save the final models at the end of training
        if i == (starting_step + nb_steps - 1):
            final_model_checkpoint_base_name = f'{data_dir}{cache_prefix}_final_model_weights.h5'
            generator_model.save_weights(final_model_checkpoint_base_name.format('generator'))
            discriminator_model.save_weights(final_model_checkpoint_base_name.format('discriminator'))
